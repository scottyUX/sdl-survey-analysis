...
%% ── Abstract ──────────────────────────────────────────────────────────────
\begin{abstract}
Generative artificial intelligence (\GenAI) tools, including software that can write code, answer questions, and assist with technical tasks, are rapidly being adopted in software engineering education, yet most empirical studies treat AI adoption as a single, uniform behavior. This paper presents an empirical analysis of how students use and engage with AI across the different phases of software development in an undergraduate software engineering course ($N = 85$). We evaluate two constructs: AI Usage (\AU), meaning how often students use AI, and AI Usage Maturity (\AUM), meaning how thoughtfully and deliberately they use it. We examine these across six phases of the software development lifecycle: planning, design, implementation, testing, deployment, and maintenance. We also examine how these relate to students' perceptions using the Technology Acceptance Model (\TAM).

Our results show that overall AU and AUM are strongly correlated ($r = 0.690$, $p < .001$), and that AUM differs significantly across development phases (Friedman $\chi^{2} = 66.13$, $p < .001$). Students show substantially higher maturity in early and middle phases than in later ones. By contrast, how often students use AI does not vary significantly across phases, meaning usage frequency is relatively uniform while the quality of AI interaction changes meaningfully. The relationship between frequency and maturity is strongest during planning ($r = 0.671$, $p < .001$) and absent during maintenance ($r = 0.164$, n.s.). These findings offer practical guidance for educators seeking to integrate AI more effectively across the full software development process.
\end{abstract}
...
The widespread availability of GenAI tools, including ChatGPT,
GitHub Copilot, Claude, and Cursor, has fundamentally changed
the landscape of software engineering education. These tools can
generate code, explain programming concepts, assist with debugging,
and support documentation, and prior research shows they can reduce
cognitive load, accelerate iteration, and increase student engagement
\cite{ali2025, becker2023, kazemitabaar2023}. At the same time,
they introduce important pedagogical concerns: AI generated outputs
may be incorrect or adopted uncritically, and overreliance without
evaluation may undermine deeper learning \cite{pirzado2024, sakib2023}.

As a result, the central question for educators has shifted from
whether students use GenAI to how they use it. Much prior empirical
work models AI adoption primarily as a unitary outcome, typically
behavioral intention, acceptance, or use, rather than as a
multidimensional set of practices \cite{taheri2025, acosta2024,
alharbi2025}. This abstraction is limiting in software engineering
education, where student work unfolds across the Software Development
Lifecycle (SDLC), the structured sequence of phases that a software
project passes through from initial planning to long-term maintenance.
In educational settings, the SDLC provides the organizational backbone
of project-based courses: students must first plan and specify
requirements, then design a system architecture, implement the code,
test for correctness and robustness, deploy the working system, and
finally maintain and improve it over time. Each of these six phases
makes different cognitive and technical demands on students, and
understanding how AI use varies across them is essential for effective
curriculum design \cite{russo2024}.

In this paper we build on the Technology Acceptance Model
(TAM) \cite{davis1989a, davis1989b} to develop a Stage-Aware TAM
framework, evaluating TAM constructs independently for each SDLC
stage. We further introduce AI Usage Maturity (AUM) to capture
not merely whether students use AI, but how, specifically whether
they engage in iterative prompting, output verification, problem
decomposition, and contextual alignment. We address the following
research questions:

\begin{description}
  \item[RQ1] How does AI usage relate to AI usage maturity across
  SDLC stages?
  \item[RQ2] How does AI usage maturity vary across SDLC stages?
\end{description}
...
TAM proposes that perceived ease of use (PEOU), meaning how easy a 
user finds a technology to operate, and perceived usefulness (PU), 
meaning how much a user believes the technology will improve their 
performance, jointly predict behavioral intention (BI), which in turn 
predicts actual use (AU)~\cite{davis1989a, davis1989b}. In simple terms, 
students are more likely to use a tool if they find it both easy to use 
and genuinely helpful. This causal chain has been validated extensively 
in educational technology research, and recent work on AI-based coding 
assistants confirms PU and PEOU remain central predictors of student 
adoption~\cite{ali2025, gupta2021, simsek2025}.

Contemporary TAM extensions incorporate context-specific antecedents 
including trust, motivation, and facilitating conditions~\cite{ali2025, 
karaoglan2023}. These extensions recognize that whether a student adopts 
a technology depends not only on their personal perceptions but also on 
broader factors such as institutional support, access to resources, and 
their confidence in the reliability of the tool. In the context of 
generative AI tools, trust is particularly important because students 
must judge not only whether a tool is easy and useful, but whether its 
outputs can be relied upon~\cite{ali2025, karaoglan2023}.

In software engineering education specifically, TAM provides a useful 
lens for understanding why students choose to engage with AI tools at 
different points in their coursework~\cite{gupta2021, simsek2025}. 
However, the standard TAM framework treats adoption as a single, uniform 
behavior, assuming that a student either accepts or rejects a technology 
in general. This assumption becomes limiting when students engage with 
the same tool across qualitatively different tasks, as is the case 
across the six phases of the SDLC. Our Stage-Aware TAM framework, 
described in Section~3, addresses this limitation by evaluating TAM 
constructs independently for each SDLC stage~\cite{davis1989a, 
davis1989b}.
...
AI literacy, which encompasses technical proficiency, critical 
evaluation, and ethical competence, has emerged as a critical 
moderator of how students engage with \GenAI systems~\cite{lee2024}. 
Researchers have defined AI literacy as the set of competencies 
that enables individuals to critically evaluate AI outputs, 
communicate effectively with AI tools, and apply AI responsibly 
in context~\cite{long2020, ng2021}. In educational settings, 
these competencies are particularly important because students 
must assess not only whether AI responses are correct, but also 
whether they are sufficiently detailed and appropriately 
contextualized for learning~\cite{nath2024}. In this study, AI 
literacy is treated as a global antecedent shaping students' 
perceptions and interaction practices across all \SDLC stages.
...
pedagogical attention. The course examined in this study follows
exactly this structure, requiring student teams to progress through
all six \SDLC phases over a full academic term at a public research
university.
...
of the student rather than the task and are treated as global antecedents in this study, though future work could examine whether their effects vary by stage.
...
self-reported; students may systematically over report mature practices,
...
Maintenance ($r = .164$, n.s.). Together with stage-level 
...
practices across the entire software lifecycle.
